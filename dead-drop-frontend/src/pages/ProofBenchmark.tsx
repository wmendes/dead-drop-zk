 ...
 * Tests the backend proof generation flow (POST /prove/ping).
 */

import { useState, useCallback } from 'react';
import { computeCommitmentNoir } from '../games/dead-drop/deadDropNoirService';
import { provePing } from '../games/dead-drop/deadDropProofService';

const DEFAULT_PROVER_URL =
  (import.meta.env.VITE_DEAD_DROP_PROVER_URL as string) || 'http://localhost:8787';

// Hardcoded test inputs (responder-only design)
// pinger_secret=(20,30), ping=(50,50) => partial=(30,20)
// responder=(22,43) => wrapped distance = 31
const PINGER_X = 20;
const PINGER_Y = 30;
const RESPONDER_X = 22;
const RESPONDER_Y = 43;
const PING_X = 50;
const PING_Y = 50;
const SESSION_ID = 12345;
const TURN = 1;
const PARTIAL_DX = (PING_X + 100 - PINGER_X) % 100;
const PARTIAL_DY = (PING_Y + 100 - PINGER_Y) % 100;
const EXPECTED_DISTANCE = 31;
...
const PHASE_DEFS = [
    { name: 'Compute commitment', description: 'Responder Poseidon2 hash in browser (client-side)' },
...
export function ProofBenchmark() {
    ...
    const run = useCallback(async () => {
        ...
        try {
            // Phase 0: Compute responder commitment (Poseidon2 in browser)
            updatePhase(0, { status: 'running' });
            const t0 = performance.now();
            const responderSalt = new Uint8Array(32);
            crypto.getRandomValues(responderSalt);
            const responderCommitment = await computeCommitmentNoir(
                RESPONDER_X,
                RESPONDER_Y,
                responderSalt
            );
            ...
            // Phase 1: Backend /prove/ping
            updatePhase(1, { status: 'running' });
            const t1 = performance.now();
            const result = await provePing(proverUrl, {
                sessionId: SESSION_ID,
                turn: TURN,
                partialDx: PARTIAL_DX,
                partialDy: PARTIAL_DY,
                responderX: RESPONDER_X,
                responderY: RESPONDER_Y,
                responderSaltHex: Buffer.from(responderSalt).toString('hex'),
                responderCommitmentHex: responderCommitment,
            });
        ...
        <div style={{ minHeight: '100vh', background: '#0a0a0f', color: '#e2e8f0', padding: '2rem', fontFamily: 'monospace' }}>
            <div style={{ maxWidth: '640px', margin: '0 auto' }}>
                ...
                <p style={{ color: '#94a3b8', fontSize: '0.875rem', marginBottom: '1.5rem' }}>
                    Tests backend proof generation via <code style={{ color: '#a78bfa' }}>POST /prove/ping</code>.
                    <br />
                    Inputs: partial=({PARTIAL_DX},{PARTIAL_DY}), responder=({RESPONDER_X},{RESPONDER_Y}), session={SESSION_ID}, turn={TURN}, random salt.
                </p>
                ...
                {summary && (
                    <div style={{ background: '#141420', border: '1px solid #16a34a', borderRadius: '0.5rem', padding: '1rem 1.25rem', marginBottom: '1.5rem' }}>
                        ...
                        <dl style={{ display: 'grid', gridTemplateColumns: 'max-content 1fr', gap: '0.25rem 1rem', fontSize: '0.875rem' }}>
                            ...
                            <dd style={{ color: summary.distance === EXPECTED_DISTANCE ? '#4ade80' : '#f87171', fontWeight: 600 }}>
                                {summary.distance}{summary.distance === EXPECTED_DISTANCE ? ' ✓' : ` ✗ (expected ${EXPECTED_DISTANCE})`}
                            </dd>
                ...
                <p style={{ marginTop: '1.5rem', fontSize: '0.75rem', color: '#334155' }}>
                    Make sure the backend is running: <code style={{ color: '#475569' }}>cd backend/dead-drop-prover && npm install && node index.js</code>
                </p>
            </div>
        </div>
    );
